//! reads newline-delimited [`MonitorSnapshotV1`] json from `kernel-spy` over a unix socket
...
fn main() -> anyhow::Result<()> {
    ...
    loop {
        ...
        let snap: MonitorSnapshotV1 = parse_export_line(line.trim()).with_context(|| {
            format!(
                "parse snapshot: {}",
                line.trim_start().chars().take(120).collect::<String>()
            )
        })?;
        println!(
